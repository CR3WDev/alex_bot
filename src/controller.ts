...
export const controller = {
  ...
  async saveWeight(
    ...
    const msgs = [
      `Peso de ${currentWeight.weight.toFixed(
        2
      )} kg salvo para o dia ${currentWeight.baseDate.toLocaleDateString()}`,
    ];
    const previousWeight = await this.lastWeight(
      currentWeight.id,
      user.id,
      baseDate
    );
    if (previousWeight) {
      const diff =
        decimalToNumber(currentWeight.weight) -
        decimalToNumber(previousWeight.weight);
      const diffStr = Math.abs(diff).toFixed(2);
      if (diff > 0) {
        msgs.push(
          `Infelizmente você ganhou peso ${diffStr} kg desde a última pesagem dia ${previousWeight.baseDate.toLocaleDateString()}`
        );
      } else if (diff < 0) {
        msgs.push(
          `Parabéns você ganhou peso ${diffStr} kg desde a última pesagem dia ${previousWeight.baseDate.toLocaleDateString()}`
        );
      } else {
        msgs.push(
          `Você manteve o seu peso desde a última pesagem dia ${previousWeight.baseDate.toLocaleDateString()}`
        );
      }
    }
